...
output_path <- '../output/' # for knit
data_path <- '../data/' # for knit
# output_path <- 'manuscript/output/' # for console
# data_path <- 'manuscript/data/' # for console
...
image_path <- paste(output_path, 'confidence.pdf', sep='') # for knit
...
```{r confidence, echo=FALSE, fig.cap="Magnitude based inference using confidence intervals", echo=FALSE}
...
image_path <- paste(output_path, "fig1a,b.pdf")
pwidth <- 7
pheight <- 3
pdf(image_path, width=pwidth, height=pheight, onefile = TRUE)
...
image_path <- paste(output_path, "fig1c.pdf")
pwidth <- 7
pheight <- 5
...
```{r plots, fig.cap = "Three methods for communicating results of an experiment. In the bar plot (A) and a Cleveland dot plot (B), the treatment effect is inferred by mentally computing the distance between treatment means. In the Harrell plot (C), the treatment effect is plotted in addition to the treatment means. Key: AA selected flies, CN control flies, F female, M male", echo=FALSE}
...
```{r make_flyBar, echo=FALSE, results='hide'}
fn <- paste(data_path, 'fly_burst.txt', sep='') # for knit
fly <- fread(fn, stringsAsFactors = TRUE)
fly[, Treatment:=factor(Treatment, c('CN', 'AA'))]
...
gg_bar <- ggplot(data=fly, aes(x=Treatment, y=Vburst, fill=Sex, group=Sex)) +
  ...
  annotate("segment", x=1+.25, y=217, xend=2+.25, yend=217, size=2) +
  annotate("text", x=1.5+.25, y=222, label=c('***'), size=14)

image_path <- paste(output_path, 'flyBar.pdf', sep='') # for knit
pwidth <- 6
pheight <- 4
pdf(image_path, width=pwidth, height=pheight, onefile = TRUE)
grid.arrange(gg_bar, nrow=1, ncol=1) 
dev.off()
```

```{r flyBar, echo=FALSE, fig.cap="Bar plot of fly data. Error bars are 1 SEM. Asterisks indicate level of significance of inference test."}
knitr::include_graphics(image_path)
```
...
image_path <- paste(output_path, 'fly_Hplot.pdf', sep='') # for knit
pwidth <- 8
pheight <- 6
pdf(image_path, width=pwidth, height=pheight, onefile = TRUE)
...
```{r flyHarrell, echo=FALSE, fig.cap="Harrell plot of fly linear model results. Error bars are 95\\% confidence intervals"}
knitr::include_graphics(image_path)
...
```{r make_mouseBarPlot, echo=FALSE, results='hide'}
...
fout <- paste(output_path, 'fake_mouse.txt', sep='')
...
fout <- paste(output_path, 'fake_mouse_long.txt', sep='')
...
image_path <- paste(output_path, 'mouseBarPlot.pdf', sep='') # for knit
pwidth <- 8
pheight <- 6
pdf(image_path, width=pwidth, height=pheight, onefile = TRUE)
...
```{r mouseBarPlot, echo=FALSE, fig.cap="Increase in body fat in the fake mouse data. (A) Percent increase. (B) Raw measures of body fat at baseline and at the end of the treatment period. Error bars are 1SE."}
knitr::include_graphics(image_path)
